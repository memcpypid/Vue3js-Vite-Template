...
<template>
  <div class="min-h-screen bg-background text-foreground flex flex-col transition-colors duration-300">
    <!-- Navbar -->
    <header class="h-16 bg-card border-b border-border shadow-sm flex items-center justify-between px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-6">
        <h2 class="text-xl font-bold tracking-tight text-primary">Vue App</h2>
        <nav class="hidden md:flex gap-4">
          <router-link to="/user" class="text-sm font-medium hover:text-primary transition-colors" active-class="text-primary">Dashboard</router-link>
          <router-link to="/user/profile" class="text-sm font-medium hover:text-primary transition-colors" active-class="text-primary">Profile</router-link>
        </nav>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="logout" class="p-2 text-rose-500 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/10 rounded-full transition-colors" title="Logout">
